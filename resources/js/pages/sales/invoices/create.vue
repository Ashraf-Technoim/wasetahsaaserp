          ...
          <form role="form" @submit.prevent="saveInvoice" @keydown="form.onKeydown($event)">
            <div class="card-body">
              <div class="row" v-if="items">
                ...
                <div class="form-group col-md-6">
                  <label for="reference">{{ $t('common.reference') }}
                    <span class="required">*</span></label>
                        ...
                        <td>
                          <div class="input-group custom-qty-input">
                            <input type="button" value="-" class="button-minus icon-shape icon-sm btn-danger"
                              data-field="quantity"
                              @click="generateItemTotal(Number(item.qty), 'qty', i - 1, 'decrement')" />
                            ...
                            <input type="number" step="any" :id="`Qty-${i}`" :value="item.qty" name="quantity"
                              ...
                              @change="generateItemTotal(Number($event.target.value), 'qty', i - 1, '')"
                              @keyup="generateItemTotal(Number($event.target.value), 'qty', i - 1, '')"
                              placeholder="Quantity" />

                            <input type="button" value="+" class="button-plus icon-shape icon-sm btn-primary"
                              data-field="quantity"
                              @click="generateItemTotal(Number(item.qty), 'qty', i - 1, 'increment')" />
                          </div>
                        </td>
                        <td>
                          <div class="input-group custom-qty-input">
              ...
              <div class="row">
                ...
                <div class="form-group col-md-4">
                  ...
                  <input id="transportCost" v-model="form.transportCost" type="number" step="any" min="1"
                    ...
                    name="transportCost" :placeholder="$t('common.transport_cost_placeholder')" @change="calculateSum"
                    @keyup="calculateSum" />
              ...
              <div class="row">
                <div v-if="taxes" class="form-group col-md-4">
                  ...
                  <v-select v-model="form.orderTax" :options="taxes" label="code"
                    ...
                    placeholder="Select a tax type" @input="calculateSum" />
  ...
  methods: {
    ...
    async getClients() {
      ...
      if (this.items && this.items.length > 0) {
        ...
        this.form.client = this.items.find(item => item.slug === defaultClientSlug);
      }
    },
    ...
    async getAccounts() {
      ...
      if (this.accounts && this.accounts.length > 0) {
        ...
        this.form.account = this.accounts.find(item => item.slug === defaultAccountSlug);
      }
    },
