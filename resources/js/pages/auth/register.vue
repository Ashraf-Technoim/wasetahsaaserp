          ...
          <div class="container">
            <div class="row">
              <div class="col-md-12 col-lg-10 col-xl-8 mx-auto">
                ...
                <form v-if="!verificationForm.email" @submit.prevent="tenantRegister" @keydown="form.onKeydown($event)">
                  <!-- Full Name-->
                  <div class="form-group mb-3">
                    <input id="name" v-model="form.name" :class="{ 'is-invalid': form.errors.has('name') }"
                      class="form-control rounded-pill border-0 shadow-sm px-4 text-primary" type="text" name="name"
                  ...
                  <div class="form-group mb-3">
                    <input v-model="form.email" id="email" name="email"
                      ...
                      class="form-control rounded-pill border-0 shadow-sm px-4 text-primary" type="email"
                  ...
                  <div class="row mb-5 ml-2">
                    <checkbox v-model="form.terms_and_conditions" id="terms_and_conditions" name="terms_and_conditions"
                      :class="{ 'is-invalid': form.errors.has('terms_and_conditions'), }" required>
                      {{ $t('register_policy') }}
                    </checkbox>
                  ...
                  <div class="row justify-content-between">
                    <router-link :to="{ name: 'find-domain' }" class="mx-2">
                      {{ $t('already_registered') }}
                    </router-link>
                    <router-link :to="{ name: 'resend' }" class="mx-2">
                ...
                <div class="mt-5" v-else>
                  <div v-if="message" class="alert" :class="type == 'success' ? 'alert-success' : 'alert-danger'">
                    ...
                    <span v-if="type != 'success'">
                      {{ $t('please') }} <router-link :to="{ name: 'find-domain' }">{{ $t('login') }}</router-link>
                    </span>
                  </div>
...
export default {
  ...
  methods: {
    ...
    async resendVerification() {
      await this.verificationForm.post('/api/email/resend')
        .then(({ data }) => {
