<template>
  <div>
        ...
        <div class="w-100">
          <div>
            <div class="row">
              ...
              <div class="col-12 col-md-6 col-xl-4 mb-2">
                <date-range-picker ref="picker" opens="center" :locale-data="locale" :minDate="minDate"
                  :maxDate="maxDate" :singleDatePicker="false" :showWeekNumbers="false" :showDropdowns="true"
                  :autoApply="true" v-model="dateRange" @update="updateValues" :linkedCalendars="true" class="c-w-100">
              ...
              <div class="col-12 col-md-12 col-xl-4 text-right mb-2">
                <div class="btn-group c-w-100">
                  ...
                  <!--upload csv button start-->
                  <a href="#" @click="uploadCsvModalShow()" class="btn btn-dark">
                  ...
                  <a href="/suppliers/pdf" v-tooltip="$t('common.export_table')" class="btn btn-secondary">
                    <i class="fas fa-file-export"></i>
                  </a>
                  <a @click="print" v-tooltip="$t('common.print_table')" class="btn btn-info">
                    <i class="fas fa-print"></i>
                  </a>
                  <router-link v-if="$can('supplier-create') &&
                  tenant.plan.limit_suppliers === 0 || pagination && tenant.plan.limit_suppliers > pagination.total"
                    :to="{ name: 'suppliers.create' }" class="btn btn-primary">
                    {{ $t("common.create") }}
    ...
    <!--csv modal start-->
    <Modal class="pay-modal" v-if="showUploadCsvModal">
        ...
        <form role="form" @submit.prevent="saveCsv" @keydown="form.onKeydown($event)">
          <div>
            <div class="form-group">
              <label for="csvFile">{{ $t('Download Demo CSV File') }}</label>
              <a href="/demo-csv-file/demo.csv">Click To Download</a>
            </div>
          </div>
          ...
          <div class="card-footer">
            ...
            <button type="reset" class="btn btn-secondary float-right" @click="form.reset()">Reset
            </button>

            <button type="button" class="btn btn-secondary" @click="uploadCsvModalClose()">{{ $t('common.close') }}
            </button>
          </div>
        </form>
      </div>

      <div class="payment-modal-footer" slot="modal-footer"></div>
    </Modal>
    <!--csv modal end-->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveCsv() {
      this.form.post('/api/supplier-import').then(() => {
