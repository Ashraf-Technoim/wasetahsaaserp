  ...
  <div>
      ...
      <div class="card-body">
        <form class="form-horizontal" @submit.prevent="update" @keydown="form.onKeydown($event)">
          ...
          <div class="form-group row">
            <label for="software_overview_section_tagline" class="col-sm-2 col-form-label text-md-right text-left">
              {{ $t('common.software_overview_section_tagline') }} <span class="required">*</span>
            </label>
            <div class="col-sm-10">
              <input type="text" v-model="form.software_overview_section_tagline" class="form-control"
                :class="{ 'is-invalid': form.errors.has('software_overview_section_tagline') }"
                id="software_overview_section_tagline"
                :placeholder="$t('common.software_overview_section_tagline_placeholder')" />
              <has-error :form="form" field="software_overview_section_tagline" />
            </div>
          </div>

          <div class="form-group row">
            <label for="software_overview_section_title" class="col-sm-2 col-form-label text-md-right text-left">
              {{ $t('common.software_overview_section_title') }} <span class="required">*</span>
            </label>
            <div class="col-sm-10">
              <input type="text" v-model="form.software_overview_section_title" class="form-control"
                :class="{ 'is-invalid': form.errors.has('software_overview_section_title') }"
                id="software_overview_section_title"
                :placeholder="$t('common.software_overview_section_title_placeholder')" />
              <has-error :form="form" field="software_overview_section_title" />
            </div>
          </div>
          ...
          <div class="table-responsive table-custom mt-3" id="printMe">
            <table class="table">
              <thead>
                <tr>
                  <th>{{ $t('common.s_no') }}</th>
                  <th>{{ $t('central.setting_images.image') }}</th>
              ...
              <tbody>
                <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                  ...
                  <td class="text-right no-print">
                    <div class="btn-group">
                      <a href="#" v-tooltip="$t('common.edit')" @click="editData(data.id)" class="btn btn-info btn-sm">
                        <i class="fas fa-edit" />
                      </a>
    ...
    <Modal v-if="showModal" @close="closeModal">
      ...
      <div class="w-100 m-0" slot="body">
        <form ref="form" @submit.prevent="save" @keydown="dataForm.onKeydown($event)" class="row">
          <!-- image -->
          <div class="form-group col-md-12">
            ...
            <div class="custom-file">
              <input id="image" type="file" class="custom-file-input" name="image"
                :class="{ 'is-invalid': dataForm.errors.has('image') }" @change="onFileChange" />
            ...
            <div v-if="url" class="bg-light mt-4 w-25">
              <img class="img-fluid" :alt="$t('common.image_alt')" :src="url" />
            </div>
          </div>
          ...
          <div class="form-group col-md-12">
            ...
            <select id="status" v-model="dataForm.status" class="form-control" name="status"
              :class="{ 'is-invalid': dataForm.errors.has('status') }">
      ...
      <div slot="modal-footer" class="d-flex w-100 justify-content-between p-3">
        <button @click="handleFormSubmit" type="submit" class="btn btn-primary">
...
export default {
  ...
  methods: {
    // get the data
    async getData() {
      const { data } = await this.form.get(window.location.origin + '/api/settings/software-overview-settings')
      this.user = data.data
    ...
    async update() {
      await this.form
        .patch(window.location.origin + '/api/settings/software-overview-settings')
        .then(() => {
          toast.fire({
            type: 'success',
    ...
    // create
    createData() {
      this.dataForm.post('/api/setting-images')
        .then(() => {
          toast.fire({ type: 'success', title: this.$t('common.success_msg') })
          this.getTableData()
    ...
    // Pagination
    async paginate() {
      this.query === '' ? await this.getTableData() : await this.searchData()
    },
    ...
    async editData(id) {
      ...
      const { data } = await axios.get(window.location.origin + '/api/setting-images/' + id)
    ...
    async deleteData(id) {
      Swal.fire({
        ...
        confirmButtonText: this.$t('central.setting_images.index.delete_confirm_button'),
      }).then((result) => {
